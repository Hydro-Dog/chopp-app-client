    ...
    if (localStream) {
      ...
      while (x < length) {
        if (!greyScale) {
          ...
          imgData.data[x] =
            alpha * (255 - imgData.data[x]) + (1 - alpha) * imgDataPrev[version].data[x];
          imgData.data[x + 1] =
            alpha * (255 - imgData.data[x + 1]) + (1 - alpha) * imgDataPrev[version].data[x + 1];
          imgData.data[x + 2] =
            alpha * (255 - imgData.data[x + 2]) + (1 - alpha) * imgDataPrev[version].data[x + 2];
          imgData.data[x + 3] = 255;
        } else {
          // GreyScale.
          var av = (imgData.data[x] + imgData.data[x + 1] + imgData.data[x + 2]) / 3;
          var av2 =
            (imgDataPrev[version].data[x] +
              imgDataPrev[version].data[x + 1] +
              imgDataPrev[version].data[x + 2]) /
            3;
